...
class Sound():
    ...
    def play_sound(event):
        ...
        # ハイライトを全て消す
        for num in range(Display.labels[0].index, Display.labels[99].index+1):
            Display.labels[num].config(bg="whitesmoke") # 背景色を白に戻す
        ...
        </speak>""".format(Azure.speech_config.speech_synthesis_voice_name, Display.texts[current_index])

        Azure.speech_synthesizer.start_speaking_ssml_async(ssml).get()
    ...
    def stop_sound(event):
        ...
        # 音声ファイルを停止
        Azure.speech_synthesizer.stop_speaking_async()
    ...
    def play_next():
        ...
        </speak>""".format(Azure.speech_config.speech_synthesis_voice_name, texts[current_index])

        # 次の音声の再生
        speech_synthesis_result = Azure.speech_synthesizer.start_speaking_ssml_async(ssml).get()
